"""RAG subgraph wiring tests — nodes mocked, the real StateGraph/edges exercised.

Two things are verified (langgraph_guide §11):
  1. route_after_audit is a pure decision function (faithful / budget logic).
  2. The compiled subgraph actually CYCLES: an unfaithful verdict loops back to
     the Researcher, and agent_max_revisions stops an always-unfaithful loop.

No live LLM/MCP — the node functions are replaced with fakes (AGENTS.md).
"""

import asyncio

from src.agents.knowledge_base import graph as graph_mod
from src.agents.knowledge_base.graph import build_rag_graph, route_after_audit
...
# --- compiled subgraph: the cycle really happens ---
